...
import { Day, ManagerViewService, PlanService, StaticDataService, Week } from '@flex-team/core';
import { DateTime } from 'luxon';
import { forkJoin, Observable, of } from 'rxjs';
import { switchMap } from 'rxjs/operators';

@Injectable()
export class ManagerDashboardResolver implements Resolve<boolean> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean | Observable<boolean> | Promise<boolean> {
    ...
    let target = this.managerViewService.selectedDate;
    this.staticDataService.targetJSDate = target.toJSDate();
    ...
    return forkJoin(targetData$).pipe(switchMap(([targetDay, targetWeek]) => {
      ...
      this.staticDataService.targetDate = DateTime.fromJSDate(new Date((<any>targetDay.workload).dayDate));
      return of(true);
    }));
  }
